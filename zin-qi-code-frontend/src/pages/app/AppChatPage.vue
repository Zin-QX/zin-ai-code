...
const fetchAppInfo = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      // 加载历史对话记录到 messages
      if (res.data.data.chatHistoryList && res.data.data.chatHistoryList.length > 0) {
        // 按 createTime 升序排列（越早的在前）
        const sorted = [...res.data.data.chatHistoryList].sort(
          (a, b) => new Date(a.createTime || 0).getTime() - new Date(b.createTime || 0).getTime()
        )
        messages.value = sorted.map((item) => ({
          type: (item.messageType === 'ai' ? 'ai' : 'user') as 'user' | 'ai',
